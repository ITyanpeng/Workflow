...
var easyuiErrorFunction = function(XMLHttpRequest) {
	...
	parent.$.messager.alert('错误', XMLHttpRequest.responseText, 'info',
			function() {
				// window.location = "/supermarket/";
			parent.$.modalDialog.openner_loginDialog.dialog("open");
		});
};
						...
						$
								.each(
										...
										function() {
											var field = this;
											bindEvent($(
													'.datagrid-body td[field=' + field + '] .datagrid-cell',
													panel));
										});
					} else {
...
$.fn.tree.defaults.loadFilter = function(data, parent) {
	...
	var idFiled, textFiled, parentField, iconClsFiled;
	if (opt.parentField) {
		idFiled = opt.idFiled || 'id';
		textFiled = opt.textFiled || 'text';
		iconClsFiled = opt.iconClsFiled || 'iconCls';
		...
		for (i = 0, l = data.length; i < l; i++) {
			if (tmpMap[data[i][parentField]]
				...
				data[i]['iconCls'] = data[i][iconClsFiled];
